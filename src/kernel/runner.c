             ...
             const char *deviceOptions, unsigned long Flags)
{
    ...
    //if (SIMMASTER(cr))//have to change to last node
    if (MASTERSIM(cr->ms))
    {
        int index_reservoir = rand() % nreservoir;
        //read_state(reservoir[index_reservoir], reservoir_state, &bReadRNG);
    }
    */
    //ENDNEW
    ...
    if (opt2bSet("-cpi", nfile, fnm))
    {
        ...
        {
            load_checkpoint(opt2fn_master("-cpi", nfile, fnm, cr), &fplog,//qwerty
