...
int cmain(int argc, char *argv[])
{
    ...
    t_filenm      fnm[] = {
        ...
        //NEW
        /*
        add new argument for mdrun -reservoir
        */
        { efCPT, "-reservoir", NULL,      ffOPTRDMULT}
        //ENDNEW
    };
#define NFILE asize(fnm)
    ...
    if (opt2bSet("-multidir", NFILE, fnm))
    {
        ...
        nmultisim = opt2fns(&multidir, "-multidir", NFILE, fnm);//qwerty
    }
    //NEW
    /*
    ...
    */
    ...
    if (nmultisim > 1)
    {
        ...
        init_multisystem(cr, nmultisim, multidir, NFILE, fnm, bParFn);//qwerty:edit this to assign 1 core for reservoir
#else
    ...
    if (opt2bSet("-cpi", NFILE, fnm))
    {
        ...
        bAppendFiles =
            read_checkpoint_simulation_part(opt2fn_master("-cpi", NFILE,//qwerty: firsttime xdr_int pop number, haven't loaded checkpoint into state
                                                          fnm, cr),
                                            &sim_part_fn, NULL, cr,
        ...
        if (MULTISIM(cr) && MASTER(cr))
        {
            ...
            check_multi_int(fpmulti, cr->ms, sim_part, "simulation part", FALSE);//qwerty: TRUE->FALSE
        }
    }
    else
    {
