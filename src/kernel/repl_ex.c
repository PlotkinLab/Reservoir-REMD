                          ...
                          real                  time)
{
    ...
    if ((re->type == ereTEMP || re->type == ereTL))
    {
        for (i = 0; i < re->nrepl; i++)
        {
            re->Epot[i] = 0;//qwerty first reset Epot to 0
        }
        bEpot              = TRUE;
        re->Epot[re->repl] = enerd->term[F_EPOT];//qwerty, Epot defined
        /* temperatures of different states*/
        for (i = 0; i < re->nrepl; i++)
        {
            re->beta[i] = 1.0/(re->q[ereTEMP][i]*BOLTZ);//qwerty beta defined
        }
    }
    else
    {
                          ...
                          t_state *state_local, gmx_large_int_t step, real time)
{
    ...
    if (bThisReplicaExchanged)
    {
        ...
        if (MASTER(cr))
        {
            ...
            }//qwerty: re->q[ereTEMP] store replica temparatures in order
