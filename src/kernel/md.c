             ...
             gmx_runtime_t *runtime)
{
    ...
    if (MASTER(cr))
    {
        if (bStateFromCP)
        {
            ...
            {
                restore_energyhistory_from_state(mdebin, &state_global->enerhist);//qwerty
            }
            else
            {
    ...
    compute_globals(fplog, gstat, cr, ir, fr, ekind, state, state_global, mdatoms, nrnb, vcm,
                    ...
                    top_global, &pcurr, top_global->natoms, &bSumEkinhOld, cglo_flags);//qwerty Epot calculated here
    if (ir->eI == eiVVAK)
    {
	...
	//ENDNEW
        ...
        else
        {
            ...
            bNStList = (ir->nstlist > 0  && step % ir->nstlist == 0);//qwerty

            bNS = (bFirstStep || bExchanged || bNStList || bDoFEP ||
                   (ir->nstlist == -1 && nlh.nabnsb > 0));//qwerty bCPT is True if bNS is True
        ...
        if (bNS && !(bFirstStep && ir->bContinuation && !bRerunMD))
        {
            ...
            else
            {
                ...
                {
                    dd_collect_state(cr->dd, state, state_global);//qwerty
                }
            }
        ...
        bGStat = (bCalcVir || bCalcEner || bStopCM ||
                  ...
                  (ir->nstlist == -1 && !bRerunMD && step >= nlh.step_nscheck));//qwerty
        ...
        if (mdof_flags != 0)
        {
            ...
            write_traj(fplog, cr, outf, mdof_flags, top_global,
                       step, t, state, state_global, f, f_global, &n_xtc, &x_xtc);//qwerty
            if (bCPT)
            {
                nchkpt++;
                bCPT = FALSE;
        ...
        while (bFirstIterate || iterate.bIterationActive)
        {
            ...
            dvdl_constr = 0;
            //NEW
            //if (!(bRerunMD && !rerun_fr.bV && !bForceUpdate) && !(opt2bSet("-reservoir", nfile, fnm) && cr->ms->sim == cr->ms->nsim-1))
            ...
            //ENDNEW
            {
                ...
                */
                ...
                update_coords(fplog, step, ir, mdatoms, state, fr->bMolPBC, f,
                              bUpdateDoLR, fr->f_twin, bCalcVir ? &fr->vir_twin_constr : NULL, fcd,
                              ekind, M, wcycle, upd, bInitStep, etrtPOSITION, cr, nrnb, constr, &top->idef);//qwerty: process this
             ...
             */
            if (bGStat || (!EI_VV(ir->eI) && do_per_step(step+1, nstglobalcomm)))//all replica halt every nstglobalcomm steps
            {
        ...
        */
        if (opt2bSet("-reservoir", nfile, fnm) && MULTISIM(cr))
        {
            if (cr->ms->sim == cr->ms->nsim-1)
            {
                while(step_rel > 0 && !do_per_step(step, repl_ex_nst) || bLastStep)
                {
       		         ...
       		         (multisim_nsteps != ir->nsteps) )
       		    {
                    ...
                    bGStat = (bCalcVir || bCalcEner || bStopCM || do_per_step(step, nstglobalcomm) || 
                        ...
                        //is True when do_per_step(step, nstglobalcomm) is True

                    if (((step_rel % gs.nstms == 0) && (multisim_nsteps < 0 ||
                           (step_rel < multisim_nsteps))) && bGStat)
                    //a simpler version is if(do_per_step(step, nstglobalcomm))
                    {
                        real     gs_buf[eglsNR];
                        if (&gs != NULL)
                        {
                        ...
                        gmx_sum_sim(eglsNR, gs_buf, cr->ms);//essential communication
                        for (i = 0; i < eglsNR; i++)
                        {
                            gs.set[i] = gs_buf[i];
                            gs.sig[i] = 0;
                ...
                /*
                one extra gmx_sum_sim have to be processed if the condition to exit the trap 
                loop is the same as the condition to perform essential communication 
                */
                if (step_rel != 0 && repl_ex_nst % nstglobalcomm == 0)
                {
                    real     gs_buf[eglsNR];
                    if (&gs != NULL)
                    {
