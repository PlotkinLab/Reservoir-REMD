                        ...
                        int n, int **v, FILE *list)
{
    ...
    res = xdr_vector(xd, (char *)vp, nf,
                     (unsigned int)sizeof(int), (xdrproc_t)xdr_int);//qwerty: Is this where information in xd transfer to vp
    if (res == 0)
    {
        return -1;
    }
                          ...
                          FILE *list)
{
    ...
    res = xdr_int(xd, &magic);//qwerty: magic will be 171817 after this command(xdr_int 1st been called)
    if (res == 0)
    {
                        ...
                        gmx_bool bReadRNG, FILE *list)
{
    ...
    nnht  = state->nhchainlength*state->ngtc;//qwerty: also defined in exchange_state in repl_ex.c
                      ...
                      gmx_large_int_t step, double t, t_state *state)
{
    ...
    fp = gmx_fio_open(fntemp, "w");//qwerty: create empty state_step####.cpt
    ...
    if (!bNumberAndKeep && !ret)
    {
        ...
        if (gmx_file_rename(fntemp, fn) != 0)//qwerty: rename state_step###.cpt to state.cpt
        {
                            ...
                            gmx_bool bAppendOutputFiles, gmx_bool bForceAppend)
{
    ...
    //qwerty: state->ngtc and nnhpres defined here
    ...
    *bReadRNG = TRUE;
    ...
    ret             = do_cpt_state(gmx_fio_getxdr(fp), TRUE, fflags, state, *bReadRNG, NULL);//qwerty
    //qwerty: gmx_fio_getxdr is defined in gmxlib/gmxfio.c, it return XDR filetype(fio->xdr)
                     ...
                     gmx_bool bAppend, gmx_bool bForceAppend)
{
    ...
    /*if (!(opt2bSet("-reservoir", nfile, fnm) && (cr->ms->sim == cr->ms->nsim-1)))
    {
                                 ...
                                 int *nfiles, gmx_file_position_t **outputfiles)
{
    ...
    do_cpt_header(gmx_fio_getxdr(fp), TRUE, &file_version,
                  ...
                  &state->edsamstate.nED, NULL);//simulation_part, step,
    ret =
...
//NEW
void read_state(const char *fn, t_state *reservoir_state, gmx_bool *bReadRNG)
{
    t_fileio            *fp;
    fp = gmx_fio_open(fn, "r");
    *bReadRNG = FALSE; // not reading the current state
    //int fflags = 0b1100111011110;//fflags is to compare with a shifted 1(1<<i) 6622
    int fflags = 6622;

    int                  simulation_part = 1;//declared right before read_checkpoint_simulation_part in mdrun.c
    gmx_large_int_t      step = 0;//declare at the beginning of read_checkpoint_simulation_part
    double               t;
    int                  file_version;
    char                *version, *btime, *buser, *bhost, *fprog, *ftime;
    int                  double_prec;
    ...
    ivec                 dd_nc;
    int                  flags_eks, flags_enh, flags_dfh;
    //init_state(&reservoir_state, 0, 0, 0, 0, 0);
    do_cpt_header(gmx_fio_getxdr(fp), TRUE, &file_version,
                  &version, &btime, &buser, &bhost, &double_prec, &fprog, &ftime,
                  &eIntegrator, &simulation_part, &step, &t, &nppnodes, dd_nc, &npme,
                  &reservoir_state->natoms, &reservoir_state->ngtc, &reservoir_state->nnhpres, &reservoir_state->nhchainlength,
                  &(reservoir_state->dfhist.nlambda), &reservoir_state->flags, &flags_eks, &flags_enh, &flags_dfh,
                  &reservoir_state->edsamstate.nED, NULL);
                  //xdr_int/xdr_double... seems has to be done in a specific sequence
                  //the number of aquiring time is only correct when do_cpt_header is called
    int ret = do_cpt_state(gmx_fio_getxdr(fp), TRUE, fflags, reservoir_state, *bReadRNG, NULL);
}
//ENDNEW
