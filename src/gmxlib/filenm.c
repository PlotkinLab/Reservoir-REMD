                       ...
                       gmx_bool bReadNode)
{
    ...
    if (deffile[fnm->ftp].ntps)//qwerty
    {
...
const char *opt2fn(const char *opt, int nfile, const t_filenm fnm[])
{
    ...
    for (i = 0; (i < nfile); i++)
    {
        ...
        {
            return fnm[i].fns[0];//qwerty: return filename
        }
    }
...
int opt2fns(char **fns[], const char *opt, int nfile, const t_filenm fnm[])
{
    ...
    for (i = 0; (i < nfile); i++)
    {
        if (strcmp(opt, fnm[i].opt) == 0)
        {
            *fns = fnm[i].fns;//qwerty: save the filenames to **fns
            return fnm[i].nfiles;//qwerty:return number of files/replicas
        }
    }
