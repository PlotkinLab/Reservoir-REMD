                      ...
                      int nfile, const t_filenm fnm[], gmx_bool bParFn)
{
    ...
    //NEW
    /*
    nnodpersim is altered according to the single core for reservoir
    */
    if (opt2bSet("-reservoir", nfile, fnm))
    {
        nnodpersim = nnodes/(nsim-1); //nnodepersim will be round off
    }
    else{
    ...
    for (i = 0; i < ms->nsim; i++)
    {
        rank[i] = i*nnodpersim;//qwerty: rank will be master node, rank = {rank, Nth core(reservoir master)}
    }
    ...
    //NEW
    /*
    allocate a single core to reservoir
    */
    if (opt2bSet("-reservoir", nfile, fnm) && cr->nodeid >= nnodpersim*(nsim-1))
    {
        cr->nnodes     = nnodes % (nsim-1);
        //cr->nnodes     = 1;
    }
    ...
    //if (opt2bSet("-reservoir", nfile, fnm))
    //{
    //   if (cr->nodeid == nnodes-1)//if it's last node
    //    cr->nnodes = 1;
    //}
    //ENDNEW
#ifdef GMX_MPI
    ...
    if (multidirs)
    {
        ...
        if (chdir(multidirs[cr->ms->sim]) != 0)//qwerty: change directory here
        {
...
t_commrec *init_par(int *argc, char ***argv_ptr)
{
    ...
    cr->sim_nodeid = gmx_setup(argc, argv, &cr->nnodes);//qwerty: (in network.c) where MPI_Comm_size and MPI_Comm_rank is called
